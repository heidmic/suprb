...
class RuleDiscoverer(ABC):
    ...
    def extract_classifier_attributes(
            self, classifiers_tuples: list[tuple[Classifier, np.ndarray]],
            x_dim: int, rho: int = None) -> np.ndarray:
        """
    ...
    def select_best_classifiers(self, tuple_list: list[tuple[Classifier, np.ndarray]],
                                mu: int) -> list[tuple[Classifier, np.ndarray]]:
        """
        ...
        for i in range(cl_num):
            ...
            for k in range(n * 2):
                new_classifier = Classifier(
                    lowers=intervals[i, :, k, 0],
                    uppers=intervals[i, :, k, 1],
...
class ES_MuLambd(RuleDiscoverer):
    ...
    def step(self, X: np.ndarray, y: np.ndarray):
        ...
        filtered_tuples = np.array(self.select_best_classifiers(
            best_nondominated_tuples, nondominated_classifiers_available), dtype=object)
    ...
    def intermediate_recombination(self, parents_tuples: list[tuple[Classifier, np.ndarray]],
                                   lmbd: int, rho: int) -> list[tuple[Classifier, np.ndarray]]:
        """
    ...
    def discrete_recombination(self, parents_tuples: list[tuple[Classifier, np.ndarray]],
                               lmbd: int, rho: int) -> list[tuple[Classifier, np.ndarray]]:
        """
    ...
    def mutate_and_fit(self, cls_tuples: list[tuple[Classifier, np.ndarray]],
                       X: np.ndarray, y: np.ndarray) -> list[tuple[Classifier, np.ndarray]]:
        """
    ...
    def replace(self, parents_tuples: list[tuple[Classifier, np.ndarray]],
                children_tuples: list[tuple[Classifier, np.ndarray]]) -> list[tuple[Classifier, np.ndarray]]:
        """
...
class ES_MuLambdSearchPath(RuleDiscoverer):
    ...
    def __init__(self, pool: list[tuple[Classifier, np.ndarray]],
                 config: dict, solution_optimizer: SolutionOptimizer = None) -> None:
        super().__init__(pool, config, solution_optimizer)
        self.sigmas = list()
    ...
    def step(self, X: np.ndarray, y: np.ndarray):
        ...
        for i in range(self.config.rule_discovery['steps_per_step']):
            ...
            for j in range(lmbd):
                ...
                cl.lowerBounds, cl.upperBounds = np.stack(
                    (start_point.lowerBounds, start_point.upperBounds) + (start_sigma * cl_sigmas))
                cl.fit(X, y)
            ...
            children_tuple_list = np.array(self.select_best_classifiers(
                rnd_tuple_list, min(len(rnd_tuple_list), mu)), dtype=object)
            ...
            search_path = (1 - sigma_coef) * search_path + np.sqrt(sigma_coef * (2 - sigma_coef)
                                                                   ) * (np.sqrt(mu) / mu) * np.sum(children_tuple_list[:, 1])
            ...
            global_factor = np.power(
                (np.exp((np.linalg.norm(search_path) ** 2 / x_dim) - 1)),
                (sigma_coef / dist_global) / 2)
    ...
    def step(self, X: np.ndarray, y: np.ndarray):
        ...
        for i in range(self.config.rule_discovery['steps_per_step']):
            ...
            for j in range(lmbd):
                ...
                cl_sigmas = Random().random.normal(size=x_dim)
                cl.lowerBounds, cl.upperBounds = np.stack(
                    (start_point.lowerBounds, start_point.upperBounds) + ((sigmas * np.sqrt(C)) @cl_sigmas))
                cl.fit(X, y)
                if (-1 <= cl.lowerBounds).all() and (cl.lowerBounds <= 1).all() and (-1 <= cl.upperBounds).all() and (cl.upperBounds <= 1).all():
                    rnd_tuple_list.append([cl, cl_sigmas])
            ...
            start_point.lowerBounds, start_point.upperBounds = np.stack(
                (start_point.lowerBounds, start_point.upperBounds) + cov_m * sigmas * C_sqrt_diag * weighted_sigma_sum)
            start_point.fit(X, y)
            # search path isotropic update
            sp_isotropic = (1 - cov_isotropic) * sp_isotropic + np.sqrt(cov_isotropic * (2 - cov_isotropic)
                                                                        ) * np.sqrt(mu_weights) * weighted_sigma_sum
            ...
            weighted_cov_sigmas_sum = np.sum(
                [children_weights[i] * C_sqrt_diag * children_tuple_list[i, 1] for i in range(min_mu)])
            ...
            weighted_cov_trans_sum = np.sum(
                [children_weights[i] * np.dot(
                    C_sqrt_diag * children_tuple_list[i, 1],
                    (C_sqrt_diag * children_tuple_list[i, 1]).T) for i in range(min_mu)])
            C = (1 - cov_one + cov_h - cov_isotropic) * C + cov_one * np.dot(sp_cov,
                                                                             sp_cov.T) + cov_mu * weighted_cov_trans_sum
