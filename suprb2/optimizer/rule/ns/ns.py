...
from .mutation import RuleMutation, Uniform, Normal
    ...
    def _optimize(self, X: np.ndarray, y: np.ndarray, n_rules: int) -> list[Rule, float]:
        ...
        if len(self.pool_) < int(self.mu / 2):
            ...
            for origin in origins:
                new_rule = self.init(mean=origin, random_state=self.random_state_)
                population.append(self.constraint(new_rule).fit(X, y))
        ...
        for i in range(1):
            ...
            for parent in parents_iterator:
                children.extend(self.crossover(parent, next(parents_iterator), random_state=self.random_state_))
            ...
            population = []
            population.extend([x[0] for x in sorted(self._calculate_novelty_score(rules=children, k=15),
                                                    key=lambda x: x[1], reverse=True)][:int(self.mu*6/7)])
            population.extend([x[0] for x in sorted(self._calculate_novelty_score(rules=parents, k=15),
                                                    key=lambda x: x[1], reverse=True)][:int(self.mu*1/7)])

        return population

    def _calculate_novelty_score(self, rules: list[Rule], k: int) -> list[Rule, float]:
        rules_with_novelty_score = []
        for rule in rules:
            distances = [jaccard(rule.match_, B.match_) for B in self.pool_]
            distances.sort()
