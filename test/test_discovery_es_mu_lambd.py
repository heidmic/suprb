...
class TestDiscoveryES_MuLambd(unittest.TestCase):
    ...
    def test_step_multiple_steps_plus(self):
        ...
        optimizer = ES_MuLambd(pool=[], config=Config())
        optimizer.step(X, y)
    ...
    def test_recombine_average(self):
        ...
        rule_disc = ES_MuLambd(pool=[], config=Config())
    ...
    def test_replace_plus(self):
        ...
        optimizer = ES_MuLambd(pool=[], config=Config())
        parents_tuples = [(Classifier(lowers=[i], uppers=[i], degree=1),
                           optimizer.create_sigmas(x_dim=1)) for i in range(5)]
        children_tuples = [(Classifier(lowers=[i], uppers=[i], degree=1),
                            optimizer.create_sigmas(x_dim=1)) for i in range(5)]
