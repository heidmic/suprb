...
class RuleMutation(GenerationOperator):
    ...
    def __init__(self,
                 ...
                 adapt_mutation: bool = False):
        super().__init__(matching_type=matching_type)
        self.sigma = sigma

        # Parameters necessery for adapting the mutation
        self.best_elitist_fitness = 0
        self.number_of_worse_iterations = 0
        self.worse_iteration_tolerance = 5
        self.adapt_mutation = adapt_mutation

    def __call__(self, rule: Rule, random_state: RandomState, adapt_mutation: bool = False) -> Rule:
    ...
    def execute(self, rule: Rule, random_state: RandomState, adapt_mutation: bool = False):
        pass
        ...
        if self.adapt_mutation:
            if elitist_fitness < self.best_elitist_fitness:
                ...
                if self.number_of_worse_iterations > self.worse_iteration_tolerance:
                    return Normal(matching_type=self.matching_type, sigma=self.sigma, adapt_mutation=True)
            else:
                self.number_of_worse_iterations = 0
...
class Normal(RuleMutation):
    ...
    def individual_mutate(self, rule: Rule, random_state: RandomState, adapt_mutation: bool = False):
    ...
    def unordered_bound(self, rule: Rule, random_state: RandomState, adapt_mutation: bool = False):
        rule.match.bounds += random_state.normal(scale=self.sigma,
                                                 size=rule.match.bounds.shape)

    def ordered_bound(self, rule: Rule, random_state: RandomState, adapt_mutation: bool = False):
    ...
    def center_spread(self, rule: Rule, random_state: RandomState, adapt_mutation: bool = False):
        self.individual_mutate(rule, random_state)

    def min_percentage(self, rule: Rule, random_state: RandomState, adapt_mutation: bool = False):
        self.individual_mutate(rule, random_state)
