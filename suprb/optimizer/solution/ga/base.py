...
class GeneticAlgorithm(PopulationBasedSolutionComposition):
    ...
    def mutation_children(self, X, y, children):
    ...
    def _optimize(self, X: np.ndarray, y: np.ndarray):
        ...
        for _ in range(self.n_iter):
            ...
            # Mutation
            mutated_children = self.mutation_children(X, y, children)
...
class SelfAdaptingGeneticAlgorithm1(GeneticAlgorithm):
...
class SelfAdaptingGeneticAlgorithm2(GeneticAlgorithm):
    ...
    def crossover_children(self, parent_pairs):
        return list(
            flatten(
                [
                    (
                        self.crossover.saga2_crossover(
                            A,
                            B,
