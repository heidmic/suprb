...
class SelfAdaptingGeneticAlgorithm(PopulationBasedSolutionComposition):
    ...
    def _optimize(self, X: np.ndarray, y: np.ndarray):
        ...
        for iter_ in range(self.n_iter):
            ...
            mutated_children = [self.mutation(child, self.mutation_rate_min, self.mutation_rate_current_max, self.fitness_mean, self.fitness_max, random_state=self.random_state_) for child in children]
    ...
    def calc_diversity(self):
        fitness_variance = np.var([i.fitness_ for i in self.population_])
        if fitness_variance > self.fitness_variance_max : self.fitness_variance_max = fitness_variance
        if fitness_variance < self.fitness_variance_min : self.fitness_variance_min = fitness_variance
