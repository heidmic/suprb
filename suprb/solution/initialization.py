...
class ZeroInit(SolutionInit):
    ...
    def __call__(self, pool: list[Rule], random_state: RandomState) -> Solution:
        return Solution(genome=np.zeros(len(pool), dtype='bool'), pool=pool, mixing=self.mixing,
...
class RandomInit(SolutionInit):
    ...
    def __call__(self, pool: list[Rule], random_state: RandomState) -> Solution:
        return Solution(genome=random(len(pool), self.p, random_state), pool=pool, mixing=self.mixing,
                        fitness=self.fitness)
